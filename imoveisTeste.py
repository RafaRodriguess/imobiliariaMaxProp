...
with st.form(key='cadastro', clear_on_submit=True):
    ...
    data_inicial = st.date_input(f'Data Contrato Inicial:')
    ...
    #         "Data_Contrato_Inicial": data_inicial.strftime('%Y/%m/%d'),
    #         "Dias_Locados": dias_locados,
    #         "Inquilino": inquilino,
    #         "Telefone": telefone
    #     } alteracao pra funcionar a busca por nome
 
    if submit_button:
        # converde direto do y/m/d la no streamlit para d/m/y
        data_inicial_formatada = data_inicial.strftime('%d/%m/%Y')
        data[select_prop][novo_contrato] = {
            "Data_Contrato_Inicial": data_inicial_formatada,
            "Dias_Locados": dias_locados,
            "Inquilino": inquilino,
            "Telefone": telefone
        }
